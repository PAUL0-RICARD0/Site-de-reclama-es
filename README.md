# Portal de Ouvidoria Municipal — Jiquiriçá/BA

> Sistema web desenvolvido por iniciativa própria para modernizar o canal de comunicação entre cidadãos e a Prefeitura Municipal de Jiquiriçá. O projeto nasceu da identificação de uma necessidade real da comunidade: um meio digital, acessível e transparente para registrar reclamações e acompanhar respostas.

---

## Sobre o Projeto

A maioria dos municípios pequenos não tem um canal digital de ouvidoria. Os cidadãos dependem de deslocamento presencial ou ligações telefônicas para registrar demandas. Este projeto resolve esse problema com uma solução web simples, funcional e de acesso imediato.

**Desenvolvido por iniciativa própria — sem encomenda, sem orientação obrigatória.**

---

## Funcionalidades

- **Registro de reclamações** — formulário intuitivo com campos validados
- **Acompanhamento de solicitações** — consulta de status por protocolo
- **Interface responsiva** — navegação clara em desktop e mobile
- **Design limpo e acessível** — foco na experiência do cidadão

---

## Tecnologias Utilizadas

| Tecnologia | Uso |
|---|---|
| HTML5 | Estrutura semântica das páginas |
| CSS3 | Estilização, layout e responsividade |
| JavaScript (Vanilla) | Manipulação do DOM, validações e interatividade |

---

## Screenshots

### Página inicial
![Página inicial](https://github.com/user-attachments/assets/54a3f954-135b-4cf7-84a6-a4e9fa2538c4)

### Registro de reclamação
![Registro de reclamação](https://github.com/user-attachments/assets/ec3da617-d000-4039-ada1-c9cc4f6e6222)

### Acompanhamento de solicitações
![Acompanhamento](https://github.com/user-attachments/assets/38a643b9-2f20-4d37-ae66-0947644484cf)

---

## Estrutura do Projeto

```
site-de-reclamacoes/
│
├── index.html              # Página inicial
├── reclamacao.html         # Registro de reclamação
├── acompanhar.html         # Acompanhamento de status
│
└── assets/
    ├── css/
    │   ├── global.css
    │   ├── inicio.css
    │   ├── reclamacao.css
    │   └── acompanhamento.css
    ├── js/
    │   ├── global.js
    │   ├── reclamacao.js
    │   └── acompanhamento.js
    └── images/
        ├── logo.jpg
        └── fundo.jpg
```

---

## Como Executar

```bash
# Clone o repositório
git clone https://github.com/PAUL0-RICARD0/site-de-reclamacoes.git

# Acesse a pasta
cd site-de-reclamacoes

# Abra no navegador
# Basta abrir o arquivo index.html — não requer servidor ou instalação
```

---

## O desenvolvimento reforçou habilidades em:
- Estruturação semântica com HTML5
- Organização de projetos multi-página
- Manipulação do DOM com JavaScript puro
- Criação de interfaces funcionais sem frameworks

---

## Autor

**Paulo Ricardo Menezes P. Santos**  
Desenvolvedor Web em Formação | Estudante de ADS — IFBA  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Paulo_Ricardo-blue?style=flat&logo=linkedin)](https://www.linkedin.com/in/paulo-ricardo-92b0613b0)  
[![GitHub](https://img.shields.io/badge/GitHub-PAUL0--RICARD0-black?style=flat&logo=github)](https://github.com/PAUL0-RICARD0)
